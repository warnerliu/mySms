...
public class TestSendSMS extends Activity {
...
/*
		// 对于得到的短信组合，无需判断每条短信的长度// 要不要进行判断呢？如果“姓名”+“电话号码”+“短信内容”超过能够发送的最大长度呢？
...
*/
		// 必须对发送的信息长度进行判断，每次发送的字符不能超过70个，超过70需要拆分发送
		if (sendText.length() > 70) {
			ArrayList<String> texts = sendSMS.divideMessage(sendText);
			for (String text : texts)
				sendSMS.sendTextMessage(toSendNumber, null, text, null, null);
		} else
			sendSMS.sendTextMessage(toSendNumber, null, sendText, null, null);
